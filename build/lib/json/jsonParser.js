 ...
 */
export function validateJson(json, validSchema, verbose = false) {
 ...
 */
function validatePrimitive(json, validSchema, verbose = false) {
    if (verbose) {
        console.log(`[INFO]: Validating primitive with type "${typeof json}".`);
    }
    if (typeof json !== validSchema.type) {
        console.warn(`[WARN]: Expected type "${validSchema.type}" but received "${typeof json}".`);
        return null;
    }
    return json;
}
 ...
 */
function validateArray(json, validSchema, verbose = false) {
    ...
    json.forEach((item, index) => {
        ...
        if (validatedItem !== null) {
            ...
            console.warn(`[WARN]: Invalid item at index "${index}". Excluding this item.`);
        }
    });
    if (validArray.length === 0) {
        console.warn(`[WARN]: No valid items found in array. Excluding this field.`);
        return null;
    }
    return validArray;
}
 ...
 */
function validateObject(json, validSchema, verbose = false) {
    ...
    if (!validSchema.properties) {
        console.warn(`[WARN]: No "properties" schema provided for object validation.`);
        return null;
    }
    const validated = {};
    let isValid = true;
 ...
 */
export function parseJson(jsonString, validSchema = null, verbose = false) {
    let parsed = null;
    if (verbose) {
