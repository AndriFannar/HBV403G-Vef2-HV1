...
export const usersApp = new Hono()
    ...
    try {
        ...
        if (!validLoginData.data) {
            return c.json({ message: 'Invalid data', errors: validLoginData.error }, StatusCodes.BAD_REQUEST);
        }
    ...
    try {
        ...
        if (!validUser.data) {
            return c.json({ message: 'Invalid data', errors: validUser.error }, StatusCodes.BAD_REQUEST);
        }
        if (await getUser(validUser.data.username)) {
            return c.json({ message: 'User already exists' }, StatusCodes.CONFLICT);
        }
        validUser.data.password = await bcrypt.hash(validUser.data.password, saltRounds);
