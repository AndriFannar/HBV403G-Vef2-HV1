...
describe('questions.db', () => {
    ...
    it('getQuestionsByCategory returns null if category not found', async () => {
        ...
        const result = await questionsDb.getQuestionsByCategory('nonexistent-category');
    ...
    it('getQuestionBySlug returns question when found', async () => {
        ...
        prisma.questions.findFirst.mockResolvedValue(mockQuestion);
    ...
    it('createQuestion creates a new question', async () => {
        ...
        prisma.questions.create.mockResolvedValue(newQuestion);
        const updatedQuestion = { ...newQuestion, slug: '42-what-is-water' };
        prisma.questions.update.mockResolvedValue(updatedQuestion);
    ...
    it('updateQuestion returns null if question not found', async () => {
        ...
        const result = await questionsDb.updateQuestion('nonexistent-slug', updatePayload);
        expect(result).toBeNull();
        expect(prisma.questions.findFirst).toHaveBeenCalledWith({
    ...
    it('updateQuestion updates and returns the question when found', async () => {
        ...
        prisma.questions.findFirst.mockResolvedValue(existingQuestion);
        const updatePayload = {
            id: 1,
            categoryId: 1,
            question: 'Updated question',
        ...
        prisma.questions.update.mockResolvedValue(updatedQuestion);
        const result = await questionsDb.updateQuestion('1-what-is-water', updatePayload);
        expect(result).toEqual(updatedQuestion);
        expect(prisma.questions.findFirst).toHaveBeenCalledWith({
            where: { slug: '1-what-is-water' },
            include: { answers: true },
    ...
    it('deleteQuestion deletes a question', async () => {
        prisma.questions.delete.mockResolvedValue(undefined);
        await expect(questionsDb.deleteQuestion('1-what-is-water')).resolves.toBeUndefined();
