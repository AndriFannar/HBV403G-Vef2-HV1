...
describe('Questions API Endpoints', () => {
    ...
    it('GET / returns all questions', async () => {
        ...
        questionsDb.getAllQuestions.mockResolvedValue(sampleQuestions);
    ...
    it('GET /:slug returns 404 if category not found', async () => {
        questionsDb.getQuestionsByCategory.mockResolvedValue(null);
    ...
    it('GET /:slug returns questions if category found', async () => {
        ...
        questionsDb.getQuestionsByCategory.mockResolvedValue(sampleQuestions);
    ...
    it('POST / returns 201 when a valid question is created', async () => {
        ...
        questionsDb.createQuestion.mockResolvedValue(createdQuestion);
        const req = new Request('http://localhost/', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    it('PATCH /:slug returns 404 if question not found', async () => {
        questionsDb.updateQuestion.mockResolvedValue(null);
        const req = new Request('http://localhost/what-is-water', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    it('PATCH /:slug returns updated question on valid update', async () => {
        ...
        questionsDb.updateQuestion.mockResolvedValue(updatedQuestion);
        const req = new Request('http://localhost/what-is-water', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    it('DELETE /:slug returns 404 if question not found', async () => {
        questionsDb.getQuestionBySlug.mockResolvedValue(null);
        const req = new Request('http://localhost/what-is-water', {
            method: 'DELETE',
        });
        const res = await questionsApp.request(req);
    ...
    it('DELETE /:slug returns 204 on successful deletion', async () => {
        ...
        questionsDb.getQuestionBySlug.mockResolvedValue(sampleQuestion);
        questionsDb.deleteQuestion.mockResolvedValue(undefined);
        const req = new Request('http://localhost/what-is-water', {
            method: 'DELETE',
        });
        const res = await questionsApp.request(req);
