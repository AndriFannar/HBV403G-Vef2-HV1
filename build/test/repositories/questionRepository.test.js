...
import { expect, test, afterEach, beforeEach, beforeAll, afterAll, } from 'vitest';
dotenv.config();
...
test('getCategory returns Categories', async () => {
    ...
    const category = await questionRepository.getCategory(undefined, question.category.name);
...
test('saveQuestion saves Question with existing category', async () => {
    ...
    await questionRepository.saveQuestion(question);
    const questions = await questionRepository.getQuestionsByCategory(categories[0].id);
